...
void sim_step(const server_config *cfg,
              int *x, int *y,
              const nacitavanie *prek)
{
    ...
    if (nx < 0 || nx >= cfg->sirka || ny < 0 || ny >= cfg->vyska) {
        return; 
    }
    if (prek && prek->mod_prekazok) {
        if (prek->prekazky[ny][nx] == 1) {
            return; 
        }
    }
...
unsigned long sim_kolko_krokov_kym_trafim(const server_config *cfg, int sx, int sy, unsigned long max_steps,const nacitavanie *prek) {
    ...
    sim_jedno_policko sim_simuluj_policko(const server_config *cfg, int sx, int sy, int K, unsigned long max_steps,const nacitavanie *prek) {
        ...
        for (int i = 0; i < cfg->R; i++)
        {
            ...
            trafil += sim_dojst_do_stredu_za_K(cfg, prek, sx, sy, K);

            suma_krokov += sim_kolko_krokov_kym_trafim(cfg,sx,sy,max_steps,prek);
    ...
    void sim_vypocitaj_maticu(const server_config *cfg, const nacitavanie *prek, policko_data *vystup_matica) {
...
void sim_interactive(const server_config *cfg,
                     sim_update_cb send_fn, void *userdata,const nacitavanie *prek)
{
    ...
    for (unsigned rep = 0; rep < R; ++rep) {
        for (int start_y = 0; start_y < cfg->vyska; ++start_y) {
            for (int start_x = 0; start_x < cfg->sirka; ++start_x) {
                if (prek && jePolickoPrekazka(prek, start_x, start_y)) continue;
                int x = start_x, y = start_y;
                unsigned kroky = 0;

                char info[128];
                snprintf(info, sizeof(info),
                         "REPLIKACIA %u/%u Z POLICKA [%d,%d]\n",
                ...
                for (unsigned k = 0; k < K; ++k) {
                    ...
                    snprintf(buf, sizeof(buf),
                             "UPDATE start_x=%d start_y=%d rep=%u krok=%u x=%d y=%d\n",
...
int jePolickoPrekazka(const nacitavanie *cfg, int x, int y)
{
    if (!cfg || !cfg->mod_prekazok)
        return 0;

    if (x < 0 || x >= cfg->sirka || y < 0 || y >= cfg->vyska)
        return 1;

    return cfg->prekazky && cfg->prekazky[y][x] == 1;
}
